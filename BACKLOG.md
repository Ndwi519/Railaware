...
1. **Vitest Worker OOM (Test Environment)**: The full client test suite occasionally triggers Out-of-Memory (OOM) errors in the Vitest worker. Suspected to be related to Leaflet/JSDOM memory accumulation; root cause not conclusively bisected across the full suite. Isolated files (confirmed: `LiveMapPage.test.jsx`) pass individually. This is accepted as a test-environment limitation and not a production issue. Tests can be run successfully in isolation (`npx vitest run <file>`).
