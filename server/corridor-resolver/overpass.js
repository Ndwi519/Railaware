import { createLogger, TopologyError } from '../utils/index.js';
import crypto from 'node:crypto';
import { extractStationFeature } from './station-helper.js';
import { haversineMetres } from '../calculations/index.js';
import { deepFreeze } from '../utils/deepFreeze.js';

const log = createLogger('corridor-resolver:overpass');
export class OverpassClient {
    ...
    async fetchNearbyRailways(location, radiusMetres) {
        ...
        if (cached) {
            if (Date.now() < cached.expiresAt) {
                if (cached.isFailure) {
                    log.info('CACHE FAILURE HIT', { requestId, cacheKey });
                    throw new TopologyError('Failed to retrieve nearby railways', { cause: cached.error });
                }
                log.info('CACHE HIT', { requestId, cacheKey });
                return cached.data;
            } else {
                log.info('CACHE EXPIRED', { requestId, cacheKey });
                this.cache.delete(cacheKey);
            }
        }

        if (this.inFlightPromises.has(cacheKey)) {
            log.info('IN-FLIGHT COALESCING HIT', { requestId, cacheKey });
            return await this.inFlightPromises.get(cacheKey);
        }

        log.info('CACHE MISS', { requestId, cacheKey });
            ...
            for (let attempt = 1; attempt <= this.config.maxAttempts; attempt++) {
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        throw new TopologyError(`Overpass API responded with status ${response.status}`, { cause: { status: response.status, body: responseBody } });
                    }
                    ...
                    try {
                        ...
                        throw new TopologyError('Failed to parse Overpass response', { cause: e });
                    }
                    
                    const { corridors, stations } = this.parseOverpassData(data);
                    const resultData = deepFreeze({ corridors, stations });
                    
                    if (data.elements && data.elements.length === 0) {
                        this.cache.set(cacheKey, { isFailure: false, data: resultData, expiresAt: Date.now() + this.config.cacheTtlNoCorridorMs });
                    } else {
                        this.cache.set(cacheKey, { isFailure: false, data: resultData, expiresAt: Date.now() + this.config.cacheTtlSuccessMs });
                    }
                    ...
                    if (isTransientFailure && attempt < this.config.maxAttempts) {
                        ...
                        log.info(`Transient failure detected, retrying in ${delay}ms... (Attempt ${attempt + 1}/${this.config.maxAttempts})`, { requestId });
                        await new Promise(resolve => setTimeout(resolve, delay));
                        continue;
                    }
                    
                    if (isTransientFailure) {
                        log.info('CACHE FAILURE MISS - Caching transient error', { requestId, cacheKey });
                        this.cache.set(cacheKey, { isFailure: true, error, expiresAt: Date.now() + this.config.cacheTtlTransientFailureMs });
                    }
                    
                    throw new TopologyError('Failed to retrieve nearby railways', { cause: error });
                }
            }
        };
    ...
    _buildCorridors(ways, nodes) {
        ...
        for (const way of ways) {
            ...
            if (points.length > 1) {
                const nodeDistanceLookup = Object.create(null);
                ...
                for (let i = 0; i < points.length; i++) {
                    ...
                    nodeDistanceLookup[String(validNodeIds[i])] = cumulativeDistance;
                    cumulativeDistances.push(cumulativeDistance);
                }
                
                const authoritativeNodeLookup = Object.create(null);
                for (const nodeId of way.nodes) {
                    authoritativeNodeLookup[String(nodeId)] = true;
                }
