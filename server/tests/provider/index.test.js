import { RailRadarProvider } from '../../provider/railradar.js';
...
describe('RailRadar Provider', () => {
    ...
    beforeEach(() => {
        provider = new RailRadarProvider({ railradarKey: 'test-key' });
    ...
    test('discoverNearbyTrains returns trains successfully', async () => {
        const mockResponse = {
            ...
            data: {
                trains: [
                    { train: { number: '123', name: 'Test Express' }, from: { departure: '10:00' }, to: { arrival: '12:00' } }
                ]
            }
        };
    ...
    test('discoverNearbyTrains throws ProviderError on malformed provider payload', async () => {
        global.fetch.mockResolvedValue({
            ...
            json: async () => ({
                success: true,
                data: {
                    trains: [ { train: { number: 123 } } ] // Missing name, which is fine, but to make it malformed let's break structure:
                }
            })
        });
    ...
    test('discoverNearbyTrains throws ProviderError on non-200 status', async () => {
        ...
        await expect(provider.discoverNearbyTrains('STN1', 'STN2')).rejects.toThrow(ProviderError);
    });
    ...
    test('getLiveTrainProgress returns topological progress successfully', async () => {
        const mockResponse = {
            ...
            data: {
                ...
                previousHalt: { stationCode: 'STN1' },
                nextHalt: { stationCode: 'STN2' },
                currentLocation: { segmentProgress: 0.75 }
            }
        };
    ...
    test('getLiveTrainProgress throws ProviderError on non-200 status', async () => {
        ...
        await expect(provider.getLiveTrainProgress('123')).rejects.toThrow(ProviderError);
    });

    test('getLiveTrainProgress throws ProviderError on malformed provider payload (Zod)', async () => {
        global.fetch.mockResolvedValue({
            ok: true,
            json: async () => ({ success: 'not-a-boolean' }) // fails schema
        });
