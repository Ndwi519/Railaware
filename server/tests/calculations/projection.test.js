var _projection = require("../../calculations/projection.js");
...
describe('projectPointOntoCorridor', () => {
  ...
  describe('Invalid Geometry', () => {
    it('returns null if point is missing or malformed', () => {
      expect((0, _projection.projectPointOntoCorridor)(null, [{
        lat: 1,
        lng: 1
      }, {
        lat: 2,
      ...
      expect((0, _projection.projectPointOntoCorridor)({
        lat: 1
      }, [{
        lat: 1,
        lng: 1
  ...
  describe('Mathematical Projection', () => {
    ...
    it('projects point directly on a node exactly', () => {
      const result = (0, _projection.projectPointOntoCorridor)({
        lat: 10.0,
        lng: 20.1
      }, corridor);
      ...
      const expectedAlongTrack = (0, _haversine.haversineMetres)(corridor[0].lat, corridor[0].lng, corridor[1].lat, corridor[1].lng);
    ...
    it('projects an offset point correctly via equirectangular approx', () => {
      ...
      expect(result.projectedPoint.lat).toBeCloseTo(10.0, 5);
      expect(result.projectedPoint.lng).toBeCloseTo(20.05, 5);
      const expectedCrossTrack = (0, _haversine.haversineMetres)(offsetLat, 20.05, 10.0, 20.05);
    ...
    it('clamps projection to segment endpoints when point is outside bounds', () => {
      ...
      const expectedCrossTrack = (0, _haversine.haversineMetres)(10.0, 19.9, 10.0, 20.0);
      expect(result.crossTrackDistanceMetres).toBeCloseTo(expectedCrossTrack, 5);
    });
    it('calculates cumulative alongTrack distance for multi-segment corridors', () => {
      ...
      expect(result.interpolationRatio).toBeCloseTo(0.5, 5);
      const seg1Len = (0, _haversine.haversineMetres)(corridor[0].lat, corridor[0].lng, corridor[1].lat, corridor[1].lng);
      const halfSeg2Len = (0, _haversine.haversineMetres)(corridor[1].lat, corridor[1].lng, corridor[2].lat, corridor[2].lng) / 2;
    ...
    it('handles zero-length segments and duplicate coordinates safely', () => {
      ...
      expect(result.interpolationRatio).toBeCloseTo(0.5, 5);
      const halfLen = (0, _haversine.haversineMetres)(10.0, 20.0, 10.1, 20.0) / 2;
      expect(result.alongTrackDistanceMetres).toBeCloseTo(halfLen, 3);

      // If we ask for projection strictly on the duplicate node
      const dupResult = (0, _projection.projectPointOntoCorridor)({
        lat: 10.0,
        lng: 20.0
      }, dupCorridor);
    ...
    it('uses deterministic tie-breaking to pick the earliest segment', () => {
      ...
      const result = (0, _projection.projectPointOntoCorridor)(middlePoint, uCorridor);
