var _stationIndex = require("../../calculations/station-index.js");
describe('buildCorridorStationIndex', () => {
  ...
  it('safely ignores invalid inputs and returns empty array', () => {
    expect((0, _stationIndex.buildCorridorStationIndex)(null, [])).toEqual([]);
    expect((0, _stationIndex.buildCorridorStationIndex)(corridor, null)).toEqual([]);
  });
  it('safely ignores malformed stations without throwing', () => {
    ...
    // malformed lat, missing lng
    {
      lat: 10.1,
      lng: 20.1
    } // missing station reference
    ];
    const index = (0, _stationIndex.buildCorridorStationIndex)(corridor, stations);
  ...
  it('applies deterministic tie-breaking for identical alongTrackDistance', () => {
    ...
    const indexReversed = (0, _stationIndex.buildCorridorStationIndex)(corridor, [stations[1], stations[0]]);
  ...
  it('safely ignores stations that fail projection without throwing', () => {
    ...
    expect(() => (0, _stationIndex.buildCorridorStationIndex)(corridor, stations)).not.toThrow();
    const index = (0, _stationIndex.buildCorridorStationIndex)(corridor, stations);
    expect(index).toHaveLength(1);
    expect(index[0].station.code).toBe('STN1');
  });
