...
describe('Logger', () => {
  ...
  test('logs info as JSON to stderr', () => {
    const logger = createLogger('test', 'corr-1');
    ...
    expect(logged.context).toEqual({ id: 1 });
  });

  test('logs error with stack trace', () => {
    const logger = createLogger('test');
    const err = new Error('oops');
    logger.error('Error occurred', err, { contextVal: 2 });
  ...
  test('child logger uses child correlationId', () => {
    const logger = createLogger('test').child('child-corr');
