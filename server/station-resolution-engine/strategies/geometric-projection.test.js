import { GeometricProjectionStrategy } from './geometric-projection.js';
...
describe('GeometricProjectionStrategy', () => {
  const validConfig = {
    ...
    maximumAlongTrackGapMetres: 1000, // 1km
    minimumStationCount: 2,
    minimumCorridorCoverage: 0.5
  };

  const validGps = { lat: 10.0000, lng: 20.0005 }; // Midpoint of first segment
  const validCorridor = [
    { lat: 10.0000, lng: 20.0000 },
    { lat: 10.0000, lng: 20.0010 }, // ~111m
    { lat: 10.0000, lng: 20.0020 }  // ~222m
  ];
  const validStations = [
    { station: { code: 'A', source: 'osm' }, lat: 10.0000, lng: 20.0000 },
    { station: { code: 'B', source: 'osm' }, lat: 10.0000, lng: 20.0020 }
  ];
  ...
  it('should initialize with correct name and configuration', () => {
    const strategy = new GeometricProjectionStrategy(validConfig);
  ...
  it('returns false for invalid gps', async () => {
    ...
    expect((await strategy.resolve({ lat: '10', lng: 20 }, validSnappedGeometry)).success).toBe(false);
  });

  it('returns false for invalid corridor geometry', async () => {
    const strategy = new GeometricProjectionStrategy(validConfig);
    expect((await strategy.resolve(validGps, null)).success).toBe(false);
    expect((await strategy.resolve(validGps, { corridorGeometry: null, stations: [] })).success).toBe(false);
    expect((await strategy.resolve(validGps, { corridorGeometry: validCorridor, stations: null })).success).toBe(false);
  });

  it('returns false for insufficient stations', async () => {
    const strategy = new GeometricProjectionStrategy(validConfig);
    const result = await strategy.resolve(validGps, { corridorGeometry: validCorridor, stations: [validStations[0]] });
  ...
  it('returns false for projection failure', async () => {
    ...
    const badCorridor = [{ lat: 'a', lng: 'b' }];
    const result = await strategy.resolve(validGps, { corridorGeometry: badCorridor, stations: validStations });
    expect(result.success).toBe(false);
  });
  ...
  it('returns false for bounding failure', async () => {
    ...
    const lateStations = [
      { station: { code: 'A', source: 'osm' }, lat: 10.0000, lng: 20.0010 },
      { station: { code: 'B', source: 'osm' }, lat: 10.0000, lng: 20.0020 }
    ];
    // Projecting at the very start of the corridor (0m). Station A is at ~111m.
    const startGps = { lat: 10.0000, lng: 20.0000 };
    const result = await strategy.resolve(startGps, { corridorGeometry: validCorridor, stations: lateStations });
    expect(result.success).toBe(false);
    expect(result.reason).toContain('outside the bounds');
  });

  it('returns false for uncalibrated constraints', async () => {
    const strategy = new GeometricProjectionStrategy({}); // uncalibrated
  ...
  it('returns false if along-track gap exceeds threshold', async () => {
    ...
    const midGps = { lat: 10.0000, lng: 20.0010 };
  ...
  it('returns false if minimum corridor coverage is not met', async () => {
    ...
    const result = await strategy.resolve(validGps, { corridorGeometry: longCorridor, stations: validStations });
    expect(result.success).toBe(false);
  ...
  it('returns successful resolution with correct properties when valid', async () => {
    ...
    expect(result.previousStation).toEqual({ code: 'A', source: 'osm' });
    expect(result.nextStation).toEqual({ code: 'B', source: 'osm' });
