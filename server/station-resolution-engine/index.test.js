...
class MockStrategy {
  constructor(name, success, previousStation = null, nextStation = null, confidence = _enums.ConfidenceLevel.MEDIUM, method = null, confidenceReasons = [], evidenceSources = []) {
    this.name = name;
...
describe('StationResolutionEngine', () => {
  ...
  beforeEach(() => {
    cache = new _cache.InMemoryResolutionCache();
  });
  test('cascades through strategies until one succeeds', async () => {
    const s1 = new MockStrategy('Fail1', false);
    const s2 = new MockStrategy('Success1', true, {
      code: 'A',
      source: 'osm'
    }, {
      code: 'B',
      source: 'osm'
    }, _enums.ConfidenceLevel.HIGH, _enums.ResolutionMethod.VERIFIED_TOPOLOGY, ['High confidence'], [_enums.EvidenceSource.OSM_STATION_NODE]);
    const s3 = new MockStrategy('Fail2', false);
    const engine = new _index.StationResolutionEngine([s1, s2, s3], cache);
    const result = await engine.resolve(gps, snappedGeometry);
    expect(result.status).toBe(_enums.ResolutionStatus.RESOLVED);
    expect(result.previousStation.code).toBe('A');
    expect(result.nextStation.code).toBe('B');
    expect(result.confidence).toBe(_enums.ConfidenceLevel.HIGH);
    expect(result.method).toBe(_enums.ResolutionMethod.VERIFIED_TOPOLOGY);
    expect(result.confidenceReasons).toEqual(['High confidence']);
    expect(result.evidenceSources).toEqual([_enums.EvidenceSource.OSM_STATION_NODE]);
  ...
  test('returns UNRESOLVED if all strategies fail', async () => {
    ...
    const engine = new _index.StationResolutionEngine([s1, s2], cache);
    const result = await engine.resolve(gps, snappedGeometry);
    expect(result.status).toBe(_enums.ResolutionStatus.UNRESOLVED);
  ...
  test('caches successful results', async () => {
    const s1 = new MockStrategy('Success1', true, {
      code: 'A',
      source: 'osm'
    }, {
      code: 'B',
  ...
  test('catches strategy exceptions and records attempt', async () => {
    ...
    const engine = new _index.StationResolutionEngine([throwingStrategy], cache);
    const result = await engine.resolve(gps, snappedGeometry);
    expect(result.status).toBe(_enums.ResolutionStatus.UNRESOLVED);
  ...
  test('propagates VERIFIED_TOPOLOGY method and provenance exactly', async () => {
    const s = new MockStrategy('VT', true, {
      ...
      code: 'B',
      source: 'osm'
    }, _enums.ConfidenceLevel.HIGH, _enums.ResolutionMethod.VERIFIED_TOPOLOGY, ['Topological match'], [_enums.EvidenceSource.OSM_STATION_NODE, _enums.EvidenceSource.OSM_TRACK_GEOMETRY]);
    const engine = new _index.StationResolutionEngine([s], cache);
    const result = await engine.resolve(gps, snappedGeometry);
    expect(result.status).toBe(_enums.ResolutionStatus.RESOLVED);
    expect(result.method).toBe(_enums.ResolutionMethod.VERIFIED_TOPOLOGY);
    expect(result.confidence).toBe(_enums.ConfidenceLevel.HIGH);
    expect(result.evidenceSources).toContain(_enums.EvidenceSource.OSM_STATION_NODE);
    expect(result.evidenceSources).toContain(_enums.EvidenceSource.OSM_TRACK_GEOMETRY);
  });
  test('propagates OFFLINE_GRAPH method and provenance exactly', async () => {
    const s = new MockStrategy('OG', true, {
      ...
      code: 'D',
      source: 'graph'
    }, _enums.ConfidenceLevel.MEDIUM, _enums.ResolutionMethod.OFFLINE_GRAPH, ['Offline graph match'], [_enums.EvidenceSource.OFFLINE_GRAPH]);
    const engine = new _index.StationResolutionEngine([s], cache);
    const result = await engine.resolve(gps, snappedGeometry);
    expect(result.status).toBe(_enums.ResolutionStatus.RESOLVED);
    expect(result.method).toBe(_enums.ResolutionMethod.OFFLINE_GRAPH);
    expect(result.confidence).toBe(_enums.ConfidenceLevel.MEDIUM);
    expect(result.evidenceSources).toEqual([_enums.EvidenceSource.OFFLINE_GRAPH]);
  });
  test('propagates PROVIDER_GRAPH method and provenance exactly', async () => {
    const s = new MockStrategy('PG', true, {
      ...
      code: 'F',
      source: 'provider'
    }, _enums.ConfidenceLevel.MEDIUM, _enums.ResolutionMethod.PROVIDER_GRAPH, ['Provider topology'], [_enums.EvidenceSource.PROVIDER_TOPOLOGY]);
    const engine = new _index.StationResolutionEngine([s], cache);
    const result = await engine.resolve(gps, snappedGeometry);
    expect(result.status).toBe(_enums.ResolutionStatus.RESOLVED);
    expect(result.method).toBe(_enums.ResolutionMethod.PROVIDER_GRAPH);
    expect(result.confidence).toBe(_enums.ConfidenceLevel.MEDIUM);
    expect(result.evidenceSources).toEqual([_enums.EvidenceSource.PROVIDER_TOPOLOGY]);
  });
  test('propagates GEOMETRIC_PROJECTION method and LOW confidence exactly', async () => {
    const s = new MockStrategy('GP', true, {
      ...
      code: 'H',
      source: 'geo'
    }, _enums.ConfidenceLevel.LOW, _enums.ResolutionMethod.GEOMETRIC_PROJECTION, ['Geometric fallback'], [_enums.EvidenceSource.GEOMETRIC_PROJECTION]);
    const engine = new _index.StationResolutionEngine([s], cache);
    const result = await engine.resolve(gps, snappedGeometry);
    expect(result.status).toBe(_enums.ResolutionStatus.RESOLVED);
    ...
    expect(result.confidence).not.toBe(_enums.ConfidenceLevel.HIGH);
  });
  test('cascades from VERIFIED_TOPOLOGY failure through to GEOMETRIC_PROJECTION success', async () => {
    ...
    const engine = new _index.StationResolutionEngine([vtFail, ogFail, pgFail, gpSuccess], cache);
    const result = await engine.resolve(gps, snappedGeometry);
    expect(result.status).toBe(_enums.ResolutionStatus.RESOLVED);
    expect(result.method).toBe(_enums.ResolutionMethod.GEOMETRIC_PROJECTION);
