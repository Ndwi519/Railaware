 ...
 */
export function calculateUserSegmentFraction(topology, nearestPointIndex) {
    const totalLengthMetres = topology.totalLengthMetres;
    
    if (totalLengthMetres <= 0) {
        return { totalLengthMetres, userSegmentFraction: 0 };
    }

    if (!Number.isInteger(nearestPointIndex) || nearestPointIndex < 0 || nearestPointIndex >= topology.cumulativeDistances.length) {
        throw new TopologyError('Invalid topology: nearestPointIndex is outside cumulativeDistances.');
    }
